...
We define a detectable size threshold in micrometers (_Rthresh_), and the time at which a colony reaches this threshold as the colony appearance time. The minimal possible Rthresh depends on the image quality and needs to be set at the same value for comparisons of tapp in different experiments. In our analysis setting, we assume that colonies
